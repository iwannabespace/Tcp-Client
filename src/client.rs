use std::{io::{self, Read, Write}, net::TcpStream, process::exit, sync::{Arc, Mutex}, time::Duration};
use std::thread;
...
impl TcpClient {
    pub fn connect(address: &str) -> Result<Self, String> {
        ...
        match data {
            Ok(data) => Ok(Self { data: Arc::new(data), nonblocking: true }),
            Err(e) => Err(e)
        }
    }
                    ...
                    Ok(size) => {
                        if size == 0 {
                            if let Ok(on_message_received) = data_ref.on_message_received.lock() {
                                let message = Message { message_type: MessageType::ConnectionDropped, body: None };
                            ...
                            if amount_to_read > 0 && read_bytes == header_size + amount_to_read {
                                if let Ok(on_message_received) = data_ref.on_message_received.lock() {
                                    if let Ok(message) = bincode::deserialize::<Message>(&buffer[header_size..]) {
                                        on_message_received(message);
                                    } else {
                                        let message = Message { message_type: MessageType::MessageDeserializeError, body: None };
                                        on_message_received(message);
                                    }
                                    buffer.resize(8, 0);
                                    read_bytes = 0;
